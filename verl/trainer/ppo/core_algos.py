...
@register_adv_est(AdvantageEstimator.CURE)
def compute_cure_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    task_type: np.ndarray | None = None,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    prefix_lens: np.ndarray | None = None,
    reward_model: np.ndarray | None = None,
    config: Optional[AlgoConfig] = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    CURE hybrid advantage. Same as GRPO outcome advantage except that:
    - For task_type == 'critique' : 1. prefix部分，advantages = 正数 2. 其余部分，advantages = token_level_rewards - baseline
    - For others, keep original GRPO outcome advantage.
    """
    
    # 1) Baseline: original GRPO scores for all rows
    scores = token_level_rewards.sum(dim=-1)  # (bs,)
    id2score = defaultdict(list)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                adj[i] = (adj[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                adj[i] = adj[i] - id2mean[index[i]]

        # Expand to token level and mask
        grpo_scores = adj.unsqueeze(-1) * response_mask  # (bs, T)

        # 2) If task_type is provided, overwrite critique rows with token-level rewards
        if task_type is not None:
            crit_mask_np = (task_type == "critique")
            experience_replay_mask = (task_type == "experience_replay")
            solve_to_replay_count = defaultdict(int)
            if np.any(experience_replay_mask):
                replay_indices = np.where(experience_replay_mask)[0]
                for i in replay_indices:
                    solve_gid = str(index[i])
                    solve_to_replay_count[solve_gid] += 1
            if np.any(crit_mask_np):
                crit_indices = np.where(crit_mask_np)[0]
                for i in crit_indices:
                    prefix_length = prefix_lens[i]
                    grpo_scores[i, :prefix_length] = 1.0
                    gt = reward_model[i]['ground_truth']
                    if gt == 1.0:
                        if scores[i] < 0:
                            grpo_scores[i, prefix_length:] = scores[i] * response_mask[i, prefix_length:]
                        else:
                            grpo_scores[i, prefix_length:] = 0.0
                            continue
                    gid_full = str(index[i])            # e.g., "abc_***"
                    solve_gid = gid_full.split("_", 1)[0]  # -> "abc"
                    solve_acc = id2mean[solve_gid]
                    if solve_gid in solve_to_replay_count:
                        replay_count = solve_to_replay_count[solve_gid]
                        solve_acc = (sum(id2score[solve_gid]) - replay_count) / (len(id2score[solve_gid])-replay_count)
                    raw_adv = (scores[i] - solve_acc)
                    grpo_scores[i, prefix_length:] = raw_adv * 2.0 * response_mask[i, prefix_length:]

    return grpo_scores, grpo_scores
